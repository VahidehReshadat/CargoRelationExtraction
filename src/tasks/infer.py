...
class infer_from_trained(object):
    ...
    def infer_one_sentence(self, sentence):
        ...
        ##########
        '''
        data_path = './dataset-SCR/Non-Directional/SCR(SM)/Train-SCR(SM).TXT'

        with open(data_path, 'r', encoding='utf8') as f:
            text = f.readlines()
        #df_ftr = pd.DataFrame(columns="Features")
        #mat = np.zeros((int(len(text) / 4), 0))
        ...
        for i in range(int(len(text) / 4)):
            ...
            print("sentttttttttttttttttttttttt")
            print(sent)
            sent=sent.strip('"')
            list_sent.append(sent)
            #mat[i,0]=sent.strip('"')
            #df_sent_ftr[i, 'Sentences']= sent


            sent=sent.replace('<e1>', '[E1]')
            sent=sent.replace('</e1>', '[/E1]')
            sent=sent.replace('<e2>', '[E2]')
            sent=sent.replace('</e2>', '[/E2]')

            self.net.eval()
            tokenized = self.tokenizer.encode(sent);  # print(tokenized)
            ...
            with torch.no_grad():
                classification_logits, v1v2 = self.net(tokenized, token_type_ids=token_type_ids,
                ...
                #df_ftr[i, ] = v1v2.tolist()

                list_ftr.append(v1v2.tolist())


        df = pd.DataFrame(list_sent)
        df2 = pd.DataFrame(list_ftr)

        print(df)
        #df2 = pd.DataFrame(list_ftr)
        #df_ftr = pd.DataFrame(mat)
        #print(df_sent_ftr.shape)
        print(df)
        print(df2)
        print(df.shape)
        print(df2.shape)
        #print(df2.iloc[100][0])
        #print(len(df2.iloc[100][0]))
        #df3=df.append(df2)
        #print(df)
        #print(df3.shape)
        df['Featrs']=df2.values

        print(df.shape)
        #print(df.iloc[100])
        df.to_csv('SCR(SM)-Train', sep='\t')
...
'''
        ...
        with torch.no_grad():
            #classification_logits, v1v2 = self.net(tokenized, token_type_ids=token_type_ids, attention_mask=attention_mask, Q=None,\
            #                            e1_e2_start=e1_e2_start)
