...
def process_text(text, mode='train'):
    ...
    for i in range(int(len(text)/4)):
        ...
        if mode == 'train':
            ...
            assert (int(re.match("^\d+", sent)[0]) - 310) == (i + 1)
...
def preprocess_fewrel(args, do_lower_case=True):
    ...
    #with open('./data/fewrel/Train_wiki_Test_wiki_Asli/train_wiki.json') as f:
    with open('./data/fewrel/Bi-jahat/FewRel(SCR-LM)/3New_Train.json') as f:
        train_data = json.load(f)
        
    #with  open('./data/fewrel/Train_wiki_Test_wiki_Asli/val_wiki.json') as f:
    with  open('./data/fewrel/Bi-jahat/FewRel(SCR-LM)/3New_Test.json') as f:
